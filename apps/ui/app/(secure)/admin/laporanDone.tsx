...
import { getCompletedReports } from '@/utils/api/admin.api';
...
export default function LaporanDone() {
  const router = useRouter();
  const { cookies } = useAuthContext();
  const [filter, setFilter] = useState('Semua');
  const [searchQuery, setSearchQuery] = useState('');

  const completedReportsQuery = useQuery<Awaited<ReturnType<typeof getCompletedReports>>>({
    queryKey: ['adminCompletedReports', cookies],
    queryFn: () => getCompletedReports(cookies),
    enabled: Boolean(cookies),
  });

  const isLoadingReports = completedReportsQuery.isPending;
  const isErrorReports = completedReportsQuery.isError;
  const errorMessage =
    completedReportsQuery.error instanceof Error
      ? completedReportsQuery.error.message
      : 'Terjadi kesalahan saat memuat laporan selesai.';

  const reports = useMemo(() => {
    const allReports = completedReportsQuery.data?.data ?? [];
    return allReports.map(mapAdminReportToHistoryItem);
  }, [completedReportsQuery.data]);
